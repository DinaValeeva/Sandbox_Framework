...
Функционал: Создание тикета
  ...
  Сценарий: Создание тикета
    ...
    * инициализация страницы "Sandbox Создание тикета"
    ...
    * инициализация страницы "Sandbox Просмотр тикета"
    * проверить, что в полях содержатся значения:
      | Очередь:                      | Django Helpdesk |
      | Электронный адрес отправителя | t@t.test        |
      | Приоритет                     | 1. Критичный    |

    #todo проверка введенных значений: дата отправки(Отправлен в), Описание, прикрепленный файл
